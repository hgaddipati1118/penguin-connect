...
class PenguinConnectReactionRequest(BaseModel):
    message_id: str = ""
    emoji: str = ""
    remove: bool = False

class PenguinConnectMessageEditRequest(BaseModel):
    message: str = ""
...
def _require_existing_conversation(conn: sqlite3.Connection, conversation_id: str) -> sqlite3.Row:
    row = conn.execute(
        """SELECT conversation_id, source_provider, source_chat_id, status, exclude_from_sync
...
def _create_scheduled_message(
    ...
    reply_context_message_id = str(req.reply_context_message_id or "").strip()
        ...
        for row in rows:
            ...
            try:
                send_result = penguinconnect_send_manual_message(
                    ...
                    reply_context_message_id=row["reply_context_message_id"] or "",
                )
            except Exception as exc:
                conn.rollback()
...
def _owned_provider_message_mutation_target(
    ...
    message = conn.execute(
        """SELECT provider, provider_message_id, body_text, metadata
           FROM penguin_connect_messages
           WHERE conversation_id = ? AND provider_message_id = ?
           LIMIT 1""",
    ...
    try:
        ...
        metadata = {}
    if metadata.get("is_from_me") is not True:
        raise HTTPException(status_code=403, detail="message_not_owned")
...
@app.patch("/penguin-connect/conversations/{conversation_id}/messages/{message_id}")
def edit_penguinconnect_message(
    conversation_id: str,
    message_id: str,
    req: PenguinConnectMessageEditRequest,
):
    text = str(req.message or "").replace("\r\n", "\n").replace("\r", "\n").strip()
    if not text:
    ...
    try:
        ...
        conn.execute(
            """UPDATE penguin_connect_messages
               SET body_text = ?, metadata = ?
               WHERE conversation_id = ? AND provider_message_id = ?""",
            (text, json.dumps(metadata), conversation_id, message_id),
        )
        conn.execute(
            """INSERT INTO penguin_connect_message_management
               (conversation_id, provider_message_id, is_starred, note, updated_at)
            ...
            (conversation_id, message_id, edited_at),
        )
        conn.commit()
        log_action(
            "api_message_edit",
...
@app.delete("/api/penguin-connect/conversations/{conversation_id}/messages/{message_id}")
@app.delete("/penguin-connect/conversations/{conversation_id}/messages/{message_id}")
def delete_penguinconnect_message(
    conversation_id: str,
    message_id: str,
):
    conn = get_connection()
    try:
        ...
        conn.execute(
            """DELETE FROM penguin_connect_message_management
               WHERE conversation_id = ? AND provider_message_id = ?""",
            (conversation_id, message_id),
        )
        conn.execute(
...
@app.post("/api/penguin-connect/conversations/{conversation_id}/messages/reaction")
@app.post("/penguin-connect/conversations/{conversation_id}/messages/reaction")
def set_penguinconnect_message_reaction(
    conversation_id: str,
    req: PenguinConnectReactionRequest,
):
    message_id = str(req.message_id or "").strip()
    emoji = str(req.emoji or "").strip()
    if not message_id:
        raise HTTPException(status_code=400, detail="reaction_message_required")
    if not emoji:
        raise HTTPException(status_code=400, detail="reaction_emoji_required")
    conn = get_connection()
    try:
        conversation = _require_schedulable_conversation(conn, conversation_id)
        source_provider = str(conversation["source_provider"] or "").strip().lower()
...
def send_penguinconnect_conversation_message(conversation_id: str, req: PenguinConnectSendRequest):
    ...
    try:
        ...
        result = penguinconnect_send_manual_message(
            ...
            reply_context_message_id=req.reply_context_message_id,
        )
        log_action(
