...
class PenguinConnectTests(unittest.TestCase):
    ...
    def test_resolve_display_name_preserves_explicit_group_title_with_handle_text(self):
        self.conn.executemany(
            """INSERT INTO contacts (first_name, last_name, organization, phone, phone_normalized, email, source_db)
               VALUES (?, ?, ?, ?, ?, ?, ?)""",
            [
        ...
        name = penguin_connect._resolve_display_name(
            self.conn,
            "Project Phoenix, +15126629638",
            ["+12405976093", "+15126629638"],
            chat_type="group",
        )

        self.assertEqual(name, "Project Phoenix, +15126629638")
    ...
    def test_refresh_contact_display_names_preserves_explicit_group_title_after_contact_import(self):
        self.conn.execute(
            """INSERT INTO penguin_connect_conversations
               (gmail_email, conversation_id, source_chat_id, display_name, chat_type, participants, alias_email, status)
               VALUES (?, ?, ?, ?, 'group', ?, ?, 'active')""",
            (
                ...
                "Project Phoenix, +15126629638",
                json.dumps(["+12405976093", "+15126629638"]),
                "owner+am-named-group@gmail.com",
            ),
        )
        self.conn.executemany(
            """INSERT INTO contacts (first_name, last_name, organization, phone, phone_normalized, email, source_db)
        ...
        self.assertEqual(row["display_name"], "Project Phoenix, +15126629638")
    ...
    def test_startup_catchup_selects_only_pending_bootstrap_conversations(self):
        ...
        self.assertEqual(selection["selected_conversations"], 1)
        self.assertEqual(selection["bootstrapped_conversations"], 0)
        self.assertEqual(selection["pending_bootstrap_conversations"], 1)
        self.assertEqual(selection["selection_strategy"], "recent_timestamp_activity")
    ...
    def test_startup_catchup_skips_already_synced_conversations(self):
        ...
        with mock.patch(
            ...
            return_value={
                ...
                "chats": [
                    {
                        "chat_id": "chat-123",
                        "first_message_at": "2026-03-15T07:10:00+00:00",
                        "last_message_at": "2026-03-15T07:15:00+00:00",
                        "message_count": 2,
                    }
                ],
            },
        ...
        self.assertEqual([row["conversation_id"] for row in conversations], [])
        self.assertEqual(selection["selected_conversations"], 0)
        self.assertEqual(selection["already_synced_conversations"], 1)
        self.assertEqual(selection["selection_strategy"], "recent_timestamp_activity")
    ...
    def test_startup_catchup_marks_due_full_verify_complete_after_success(self):
        ...
        self.assertTrue(stats["success"])
        self.assertEqual(stats["selection_strategy"], "recent_timestamp_activity")
        self.assertEqual(stats["selected_conversations"], 1)
        self.assertEqual(imessage_calls, [("amc_test", False)])
        self.assertEqual(gmail_calls, [("amc_test", False, "owner@gmail.com", ("owner@gmail.com",))])
