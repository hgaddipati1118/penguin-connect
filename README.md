...
persistent per-thread reply drafts, new chat draft staging, and a Codex prompt
helper that copies recent thread context, local notes, tags, and your draft for
review. Use `message-search` to search synced message bodies in the local bridge
cache, or search Apple Messages text directly with `--source imessage` or
`--source both`. The `messages` command shows imported attachment summaries,
including audio attachments from Apple Messages.
