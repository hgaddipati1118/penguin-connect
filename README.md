...
mark-read/mark-unread/pin-unpin/mute-unmute/archive-restore/label add-remove, selected-people add/copy/save/create, and draft cleanup actions, latest-message previews in the conversation rail,
highlighted conversation search,
global synced-message search with highlighted note/attachment-aware results, recent-message browsing, open/copy/reply actions and filters for current thread, unread, sent,
...
message notes, per-message read/unread toggles, visible loaded-message copy and bulk star/read/unread actions, opening synced attachment files, a per-thread media browser for loaded
images/audio/files, inline previews for local image attachments, inline playback
for local audio attachments, highlighted cached Contacts search/refresh with saved/unsaved/favorite/noted
source filters with counts and contact sorting, all/saved/unsaved contact browsing, conversation-participant fallback and related-thread shortcuts with unread/source context,
thread-name and local-management-aware contact/participant search with highlighted related-thread label/note context and contact-to-thread rail filtering, private contact notes with note-aware search, contact creation, contact-to-thread matching, saved-contact resolution for
...
search/new-chat/contact creation, participant favoriting, bulk add-all, copy-all, and save-as-list actions, local thread titles, notes, tags and muted state,
