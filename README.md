...
mark-read/archive actions, global synced-message search, opening synced
...
with the `--source imessage` or `--source both` options. The `messages` command
shows imported attachment summaries, including audio attachments from Apple
Messages.
